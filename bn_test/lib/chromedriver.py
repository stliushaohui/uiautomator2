    ...
    def driver(self, device_ip=None, package=None, attach=True, activity=None, process=None):
        ...
        capabilities = {
            'chromeOptions': {
                ...
                'androidPackage': package or self._d.info['currentPackageName'],
                'androidUseRunningApp': attach,
                'androidProcess': process or package or self._d.info['currentPackageName'],
                # 'androidActivity':'com.bfsggame.AppActivity'
        ...
        try:
            # dr = webdriver.Remote('http://localhost:%d' %
                                  # self._port, capabilities)
            dr = webdriver.Remote('http://localhost:%d' %
                                  self._port, capabilities)
        except URLError:
